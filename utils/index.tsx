...
export default function GameofTheDay() {
  ...
  const visibleGamesByHour = useMemo(() => {
    ...
    return groups;
  }, [games, selectLeagues, teamSelectedId, windowWidth, activeFilter]);
  ...
  const getGamesFromApi = useCallback(async (dateToFetch: Date) => {
    ...
    // 2. Lance la requête API, puis MAJ le state à la réponse (même si le cache existe)
    try {
        ...
        <div style={{ position: 'sticky', top: 0, zIndex: 10 }}>
          <ThemedView>
            <div style={{ position: 'relative', zIndex: 20 }}>
              ...
              <FilterSlider selectedFilter={activeFilter} onFilterChange={handleFilterChange} />
              {displayFilters()}
            </div>
